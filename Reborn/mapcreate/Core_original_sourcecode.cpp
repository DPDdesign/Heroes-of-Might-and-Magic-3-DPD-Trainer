...
private:
	...
	int	isize[12] = { 3, 3, 1, 5, 5,  3,  3, 3, 3, 3, 3,  1 };						/// Amount of signs per element	(127 heroes = 3 signs, maximum 30 000 creatures -  5 signs, 1 spell bool 0/1 )
	...
	int buffor[64][20];																/// output buffor 8 players * 8 heroes * 12 parameters per hero
	int hero_no;																	/// Number of hero
	int wsize[12] = { 1,1,1,2,2,1,1,1,1,1,1,1 };
...
void gracz::getdata(int n, FILE* m)
{
	...
	input = m;																		/// Assigning the input file
																					...
																					/// LOCAL VARIABLES 
	int herloc = n * 297;															/// Offset in input file reading
	...
	for (int i = 0; i < 11; i++) {
		for (int j = 0; j < icount[i]; j++) {
			...
			// cout << "buffor " << i << ", " << j+1 << " : " << buffor[i][j] << endl;
		}
	...
	for (int r = 0; r < 12; r++)
	{
		for (int i = 0; i < ecount[r]; i++)
		{
			...
			if (hexx.size() == 4) {													/// switch hex values (as the processor reats bytes backwards)
				...
				fseek(output, locm, 0);
				fwrite(&buffor[r][i], 1, wsize[r], output);
			}
		}
	}
}
...
string ExePath() {
	...
	string temp = string(buffer).substr(0, pos);
...
int main() {
	...
	string diri = ExePath();
	string diro = ExePath();
	string inputfile = diri.append("\\TA_Resources\\docore.txt");
	string outputfile = diro.append("\\random_maps\\mapka.h3m");
	const char* inn = diri.c_str();
	const char* outt = diro.c_str();

	/// READING DATA FROM INPUT FILE
	FILE* input;
	input = fopen(inn, "r+");												/// Open input file
	...
	FILE* output;
	output = fopen(outt, "rb+");												/// Open map file
